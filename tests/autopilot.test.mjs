...
test('אישור פתרון מזיז אירוע מתנגש ומעדכן בקשת הסעה', () => {
  const data = autopilot.runAutopilotScenario(fresh(), 'Avrahami', 'basketball').data
...
test('איחור, שינוי שעת סיום וביטול מעדכנים נתונים אמיתיים', () => {
  let data = autopilot.runAutopilotScenario(fresh(), 'Avrahami', 'basketball').data
  ...
  assert.equal(autopilot.runAutopilotScenario(cancelled.data, 'Avrahami', 'cancel').applied, false)
})
...
test('תרחיש במשפחה אחת אינו משנה תא משפחתי אחר', () => {
  ...
  assert.equal(result.data.events.filter(item => item.familyId === 'Avrahami').length, data.events.length)
