...
import { ageFromBirthYear, dateLabel, detectScenario, initialData, localDate, readData, removePersonAndTheirData, sanitizeAppData, uid, type AppData, type FamilyEvent, type FamilyPreferences, type FamilyTask, type FamilyUnit, type Person, type Priority, type TransportationRequest } from './data'
...
function App() {
  ...
  const familyDecisionCount = new Set([...familyOpenRequests.map(request => request.eventId), ...familyEvents.filter(event => event.needsAttention && !event.responsibleId && event.date >= localDate()).map(event => event.id), ...conflictedEvents.filter(event => event.date >= localDate()).map(event => event.id), ...futureRisks.map(risk => risk.eventId), ...familyTasks.filter(task => !task.done && !task.ownerId).map(task => `task:${task.id}`)]).size
  ...
  const familyTodayRides = familyEvents.filter(event => event.date === localDate() && event.requiresDriver).length
  const statusDetails = familyDecisionCount ? [countLabel(familyDecisionCount, 'החלטה פתוחה', 'החלטות פתוחות'), ...(familyRidesWithoutDriver ? [countLabel(familyRidesWithoutDriver, 'הסעה ללא נהג', 'הסעות ללא נהג')] : []), ...(familyConflicts ? [countLabel(familyConflicts, 'התנגשות צפויה', 'התנגשויות צפויות')] : [])].slice(0, 3).join(' · ') : `${countLabel(familyTodayEvents, 'אירוע היום', 'אירועים היום')} · ${countLabel(familyTodayRides, 'הסעה', 'הסעות')} · אין בעיות פתוחות`
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      const result = advanceAutomaticExternalScenarios(dataRef.current, family.id, actorId)
  ...
  }, [family.id, family.people, activePersonId])
  useEffect(() => {
    if (!family.people.some(person => person.age < 18)) return
    const check = () => {
      if (document.visibilityState !== 'visible') return
      const result = advanceAutomaticScenarios(dataRef.current, family.id)
      if (!result.applied) return
      dataRef.current = result.data
      setData(result.data)
      setToast(result.message)
  ...
  function openEvent(item?: FamilyEvent) {
    ...
    setForm(item ? { title: item.title, date: item.date, time: item.time, endTime: item.endTime || '', icon: item.icon, responsibleId: item.responsibleId, passengerId: requestForEvent(data, item.id)?.passengerId || item.participantIds[0] || '', details: item.details, requiresDriver: item.requiresDriver ? 'true' : 'false', priority: item.priority || 'normal', preferredDriverId: item.preferredDriverId || '', transitAvailable: String(!!item.transitAvailable) } : { title: '', date: localDate(), time: '17:00', endTime: '', icon: '📅', responsibleId: '', passengerId: family.people.find(person => person.age < 18)?.id || activePersonId, details: '', requiresDriver: 'false', priority: 'normal', preferredDriverId: '', transitAvailable: 'false' })
    setParticipants(item?.participantIds || (activePersonId ? [activePersonId] : [])); setDialog({ type: 'event', item })
  }
  function openTask(item?: FamilyTask) { if (childMode) { setToast('עריכת משימות זמינה בתצוגת מבוגר'); return }; setForm(item ? { title: item.title, due: item.due, ownerId: item.ownerId, requiresAdult: item.requiresAdult ? 'true' : 'false', priority: item.priority || 'normal', flexible: String(item.flexible !== false) } : { title: '', due: localDate(), ownerId: activePersonId, requiresAdult: 'false', priority: 'normal', flexible: 'true' }); setDialog({ type: 'task', item }) }
  function openPerson(item?: Person) { if (childMode) { setToast('ניהול המשפחה זמין בתצוגת מבוגר'); return }; setForm(item ? { name: item.name, role: item.role, color: item.color, age: String(item.age), birthYear: item.birthYear ? String(item.birthYear) : '', hasLicense: String(item.hasLicense), hasCar: String(item.hasCar), availableForPickup: String(item.availableForPickup), availability: item.availability || 'available', unavailableUntil: item.unavailableUntil || '' } : { name: '', role: 'בן', color: palette[family.people.length % palette.length], age: '', birthYear: '', hasLicense: 'false', hasCar: 'false', availableForPickup: 'false', availability: 'available', unavailableUntil: '' }); setDialog({ type: 'person', item }) }
  ...
  function updateForm(key: string, value: string) { setForm(previous => ({ ...previous, [key]: value, ...(key === 'birthYear' && /^\d{4}$/.test(value) ? { age: String(ageFromBirthYear(Number(value))) } : {}) })) }
    ...
    if (dialog.type === 'event') {
       ...
       setData(previous => {
         const next = saveEventAndDependents(previous, event)
          ...
          return ensureRequests({ ...next, transportationRequests, events: changedNeed && event.requiresDriver ? next.events.map(item => item.id === event.id ? { ...item, responsibleId: '', needsAttention: true } : item) : next.events, activity: [log(`${dialog.item ? 'עודכן' : 'נוסף'} אירוע: ${event.title}`, [activePersonId]), ...next.activity] }, activePersonId)
       })
      ...
      const task: FamilyTask = { id: dialog.item?.id || uid(), familyId: family.id, title: form.title.trim(), ownerId: form.ownerId || '', due: form.due, done: dialog.item?.done || false, eventId: dialog.item?.eventId, requiresAdult: form.requiresAdult === 'true', priority: form.priority as Priority, flexible: form.flexible === 'true' }
  ...
  function removeTask(item: FamilyTask) {
    askConfirmation(`למחוק את המשימה "${item.title}"?`, () => { setData(previous => ({ ...previous, tasks: previous.tasks.filter(t => t.id !== item.id) })); setDialog(null); setToast('המשימה נמחקה') })
  }
  ...
  function removeFamily(item: FamilyUnit) {
    ...
    askConfirmation(`למחוק את "${item.name}" ואת כל האירועים והמשימות שלו?`, () => {
      ...
      setData(previous => ({ families: previous.families.filter(f => f.id !== item.id), events: previous.events.filter(e => e.familyId !== item.id), tasks: previous.tasks.filter(t => t.familyId !== item.id), activity: previous.activity.filter(a => a.familyId !== item.id), transportationRequests: previous.transportationRequests.filter(r => r.familyId !== item.id), integrationLogs: previous.integrationLogs.filter(entry => entry.familyId !== item.id), calendarMirrors: previous.calendarMirrors.filter(entry => entry.familyId !== item.id) }))
...
function FamilyPreferencesPanel({ family, onPreferences, onPerson }: { family: FamilyUnit; onPreferences: (changes: FamilyPreferences) => void; onPerson: (id: string, changes: Partial<Person>) => void }) {
  ...
  const rules: { key: keyof FamilyPreferences; label: string; defaultValue: boolean }[] = [
...
function IntegrationHub({ data, familyId, actorId, compact = false, onRun, onShowAll }: { data: AppData; familyId: string; actorId: string; compact?: boolean; onRun: (source: ExternalScenarioId) => void; onShowAll?: () => void }) {
  ...
  return <section className="section-card integration-hub">
    ...
    {visibleLogs.length ? <div className="integration-log-list">{visibleLogs.map(entry => <div className="integration-log" key={entry.id}><span>{externalScenarios.find(scenario => scenario.source === entry.source)?.icon || sources.find(source => source.id === entry.source)?.icon || '👨‍👩‍👧‍👦'}</span><div><strong>{integrationDisplayText(entry.action)}</strong>{entry.trigger === 'automatic' && <em>זוהה אוטומטית</em>}<small>{integrationDisplayText(entry.sourceText)}</small>{data.calendarMirrors.some(mirror => mirror.eventId === entry.eventId) && <em>נוסף ליומן גוגל</em>}</div></div>)}</div> : <p className="integration-empty">עדיין אין עדכונים. אפשר להפעיל תרחיש במרכז העדכונים.</p>}
  </section>
}
...
function ActivityFeed({ data, family }: { data: AppData; family: FamilyUnit }) {
  ...
  return <section className="section-card family-feed"><div className="section-heading"><div><span className="section-kicker">הפעילות במשפחה</span><h2>מה קרה היום</h2></div></div>{entries.length ? <div className="feed-list">{entries.map(entry => <div className="feed-row" key={entry.id}><time dateTime={entry.createdAt}>{new Intl.DateTimeFormat('he-IL', { hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }).format(new Date(entry.createdAt))}</time><div><strong>{integrationDisplayText(entry.text)}</strong><small>{entry.source === 'family' ? 'אוטופיילוט משפחתי' : entry.source ? integrationNames[entry.source] : names.get(entry.personId || '') || 'המשפחה'}</small></div></div>)}</div> : <Empty text="עדיין לא היו עדכונים היום."/>}</section>
}
