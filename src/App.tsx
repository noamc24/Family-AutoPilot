...
function App() {
  ...
  const statusDetails = familyDecisionCount ? openIssues.join(' · ') : `${countLabel(familyTodayEvents, 'אירוע היום', 'אירועים היום')} · ${countLabel(familyTodayRides, 'הסעה', 'הסעות')} · אין בעיות פתוחות`
  ...
  function saveForm(confirmed = false) {
    ...
    if (dialog.type === 'event') {
      ...
      if (routines.some(routine => !routine.label.trim() || routine.day < 0 || !routine.start || !routine.end || routine.start >= routine.end)) { setToast('יש להשלים יום ושעות לכל פריט בלו״ז הקבוע'); return }
...
function WeeklySchedule({ family, actorId, scope }: { family: FamilyUnit; actorId: string; scope: 'mine' | 'family' }) {
  const rows = family.people.filter(person => scope === 'family' || person.id === actorId).flatMap(person => (person.routines || []).map(routine => ({ person, routine }))).sort((a, b) => a.routine.day - b.routine.day || a.routine.start.localeCompare(b.routine.start))
  return <section className="section-card weekly-schedule"><div className="section-heading"><div><span className="section-kicker">שעות שחוזרות מדי שבוע</span><h2>לו״ז קבוע</h2></div></div>{rows.length ? <div className="weekly-rows">{rows.map(({ person, routine }) => <div key={`${person.id}:${routine.id}`}><strong>{routineDays[routine.day]} · {routine.start}–{routine.end}</strong><span>{routine.label} · {person.name}</span>{routine.prepTitle && <small>הכנה: {routine.prepTitle}</small>}</div>)}</div> : <Empty text="עדיין לא הוגדר לו״ז קבוע. אפשר להוסיף אותו בעריכת בן משפחה."/>}</section>
}
