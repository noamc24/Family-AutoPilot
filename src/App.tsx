...
import { closureIssues, formatRoutineDayRange, materializeRoutineTasks, nextRepeatDate, routineAt, routineConflictingEvents, routineDays, routineDaysList, routineGroupKey, sensitiveAutomaticChange, syncAcknowledgements } from './workflow'
  ...
  function saveForm(confirmed = false) {
    ...
    if (dialog.type === 'event') {
      ...
      const routineDaysSelected = routines.map(routine => {
        const selected = Array.isArray(routine.days)
          ? [...new Set(routine.days.filter((day): day is number => typeof day === 'number' && Number.isInteger(day) && day >= 0 && day < 7))]
          : typeof routine.day === 'number' && Number.isInteger(routine.day) && routine.day >= 0 && routine.day < 7
            ? [routine.day]
            : []
        return { ...routine, days: selected, day: selected[0] ?? 0 }
      })
...
function RoutineEditor({ routines, people, onChange }: { routines: WeeklyRoutine[]; people: Person[]; onChange: (value: WeeklyRoutine[]) => void }) {
  ...
  const selectedDays = (item: WeeklyRoutine) => Array.isArray(item.days) && item.days.length ? item.days : Number.isInteger(item.day) && item.day! >= 0 ? [item.day!] : []
  const add = (kind: RoutineKind) => onChange([...routines, { id: uid(), kind, label: kinds.find(item => item.id === kind)!.example, day: -1, days: [], start: '', end: '', prepTitle: kind === 'study' ? 'להכין תיק לימודים' : kind === 'activity' ? 'להכין ציוד לפעילות' : '' }])
