...
import { alternativeForRequest, applyAlternativePlan, confirmDriver, ensureRequests, reconcileTransportation, recommendDriver, requestForEvent, respondToRequest } from './coordination'
import { advanceAutomaticIntegrations, detectIntegrationScenario, integrationNames, simulateIntegration, type IntegrationScenario } from './integrations'
...
function App() {
  ...
  const attentionCount = attention.length + attentionTasks.length + actionableRequests.length + unresolvedRequests.length + conflictedEvents.filter(event => !requestForEvent(data, event.id)).length
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      const result = advanceAutomaticIntegrations(dataRef.current, family.id, actorId)
  ...
  function openEvent(item?: FamilyEvent) {
    ...
    setForm(item ? { title: item.title, date: item.date, time: item.time, endTime: item.endTime || '', icon: item.icon, responsibleId: item.responsibleId, passengerId: requestForEvent(data, item.id)?.passengerId || item.participantIds[0] || '', details: item.details, requiresDriver: item.requiresDriver ? 'true' : 'false' } : { title: '', date: localDate(), time: '17:00', endTime: '', icon: '📅', responsibleId: '', passengerId: family.people.find(person => person.age < 18)?.id || activePersonId, details: '', requiresDriver: 'false' })
    setParticipants(item?.participantIds || (activePersonId ? [activePersonId] : [])); setDialog({ type: 'event', item })
  }
  function openTask(item?: FamilyTask) { if (childMode) { setToast('עריכת משימות זמינה בתצוגת מבוגר'); return }; setForm(item ? { title: item.title, due: item.due, ownerId: item.ownerId, requiresAdult: item.requiresAdult ? 'true' : 'false' } : { title: '', due: localDate(), ownerId: activePersonId, requiresAdult: 'false' }); setDialog({ type: 'task', item }) }
...
function DecisionCenter({ data, family, actorId, onRespond, onConfirm, onFind }: { data: AppData; family: FamilyUnit; actorId: string; onRespond: (id: string, response: 'CAN_DO' | 'CANNOT_DO') => void; onConfirm: (id: string, driverId: string) => void; onFind: (eventId: string) => void }) {
  const issues = data.events.filter(event => event.familyId === family.id && (scheduleConflicts(data, event).length > 0 && !!event.createdById || !!requestForEvent(data, event.id) && requestForEvent(data, event.id)?.status !== 'COVERED')).reverse().slice(0, 6)
  if (!issues.length) return null
  const name = (id: string) => family.people.find(person => person.id === id)?.name || 'בן משפחה'
  return <section className="section-card decision-center" id="decisions"><div className="section-heading"><div><span className="section-kicker">האוטופיילוט זיהה</span><h2>דורש החלטה</h2></div><span className="count-badge">{issues.length}</span></div><div className="decision-list">{issues.map(event => {
...
function IntegrationHub({ data, familyId, actorId, compact = false, onRun, onShowAll }: { data: AppData; familyId: string; actorId: string; compact?: boolean; onRun: (source: IntegrationScenario) => void; onShowAll?: () => void }) {
  ...
  return <section className="section-card integration-hub"><div className="section-heading"><div><span className="section-kicker">עדכונים ממקורות</span><h2>{compact ? 'מה התעדכן בשבילך' : 'מרכז העדכונים'}</h2></div>{compact && <button className="text-link" onClick={onShowAll}>לכל העדכונים <ArrowLeft size={15}/></button>}</div>{compact ? <div className="integration-source-chips">{sources.map(source => <span key={source.id}>{source.icon} {integrationNames[source.id]}</span>)}</div> : <><p className="integration-intro">אפשר לבדוק מקור עכשיו. עדכון חדש יופיע בלוח המשפחתי.</p><div className="integration-sources">{sources.map(source => <div className="integration-source" key={source.id}><span>{source.icon}</span><strong>{integrationNames[source.id]}</strong><small>{source.description}</small><button className="secondary-button" onClick={() => onRun(source.id)}>בדוק עכשיו <ArrowLeft size={14}/></button></div>)}</div></>}{visibleLogs.length ? <div className="integration-log-list">{visibleLogs.map(entry => <div className="integration-log" key={entry.id}><span>{sources.find(source => source.id === entry.source)?.icon || '👨‍👩‍👧‍👦'}</span><div><strong>{integrationDisplayText(entry.action)}</strong>{entry.trigger === 'automatic' && <em>זוהה אוטומטית</em>}<small>{integrationDisplayText(entry.sourceText)}</small>{data.calendarMirrors.some(mirror => mirror.eventId === entry.eventId) && <em>נוסף ליומן גוגל</em>}</div></div>)}</div> : <p className="integration-empty">עדיין אין עדכונים. אפשר לבדוק מקור עכשיו, ועדכונים יופיעו גם אוטומטית.</p>}</section>
}
...
function RequestBoard({ requests, data, actorId, onRespond, onConfirm, onAlternative, onWithdraw }: { requests: TransportationRequest[]; data: AppData; actorId: string; onRespond: (id: string, response: 'CAN_DO' | 'CANNOT_DO') => void; onConfirm: (id: string, driverId: string) => void; onAlternative: (id: string) => void; onWithdraw: (id: string) => void }) {
